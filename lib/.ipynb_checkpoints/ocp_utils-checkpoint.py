...
def extract_cost_from_mpc(mpc):
    '''
    Extract the cost from mpc for each cost term
    '''
    costs = dict()
    models, datas = np.concatenate([mpc._problem.runningModels, [mpc._problem.terminalModel]]),\
                    np.concatenate([mpc._problem.runningDatas, [mpc._problem.terminalData]]) 

    for model_i,data_i in zip(models, datas):
        if type(model_i) is crocoddyl.IntegratedActionModelEuler:
            model_i = model_i.differential
            data_i = data_i.differential
            
        keys = model_i.costs.costs.todict()
        for key in keys:
            if key not in costs:
                costs[key] = []
                
            if model_i.costs.costs[key].active:
                costs[key] += [data_i.costs.costs[key].cost]
            else:
                costs[key] += [0]
