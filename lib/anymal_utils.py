...
def compute_ik(rmodel, rdata, q0, foot_pose_d, rcond=1e-5):
    foot_pose = compute_foot_pose(rmodel, rdata, q0)    
    cur_r = foot_pose-foot_pose_d
    J = compute_foot_jacobian(rmodel, rdata, q0)[:,6:]
    # dq = np.linalg.inv(J.T@J)@J.T@cur_r
    # dq = np.linalg.solve(J, cur_r)
    # dq = np.linalg.lstsq(J, cur_r, rcond=rcond)[0]
    dq = np.linalg.pinv(J, rcond=rcond).dot(cur_r)
    dq = np.concatenate([np.zeros(7), dq])
    q = q0-dq
    return q

def compute_ik_traj(rmodel, rdata, q0, foot_pose_set, com_traj = None, rcond=1e-5):
    qs = []
    q_prev = q0.copy()
    for i in range(len(foot_pose_set)):
        if com_traj is not None:
            q_prev[:7] = com_traj[i].copy()
        q = compute_ik(rmodel, rdata, q_prev, foot_pose_set[i], rcond=rcond)
        qs.append(q)
        q_prev = q.copy()
    return np.array(qs)
...
def computeJacobian(rmodel,rdata,ee_frame_id,q):
    ...
    J = pinocchio.getFrameJacobian(rmodel, rdata,ee_frame_id, pinocchio.ReferenceFrame.LOCAL_WORLD_ALIGNED)
    return J

def compute_foot_jacobian(rmodel,rdata,q):
    ee_frame_id_list = [rmodel.getFrameId(contact_name) for contact_name in contact_names]
    pinocchio.forwardKinematics(rmodel,rdata,q)
    pinocchio.updateFramePlacements(rmodel,rdata)
    pinocchio.computeJointJacobians(rmodel, rdata, q)
    J = []
    for ee_frame_id in ee_frame_id_list:
        J += [pinocchio.getFrameJacobian(rmodel, rdata,ee_frame_id, pinocchio.ReferenceFrame.LOCAL_WORLD_ALIGNED)[:3]]
    return np.vstack(J)

def calculate_vel(rmodel, qs, dt=0.01):
    dqs = [pinocchio.difference(rmodel, q1, q2)/dt for q1,q2 in zip(qs[:-1],qs[1:])]
    dqs +=[dqs[-1].copy()]
    return np.vstack(dqs)

def check_traj(qs,final_config_threshold=2., final_z_threshold=0.6):   
    qs = qs[:,:19]
    q0,qT = qs[0], qs[-1]
    
    if np.linalg.norm((qT-q0)[7:]) > final_config_threshold:
        return False
    
    if np.linalg.norm((qT-q0)[2]) > final_z_threshold:
        return False
...
#             contacts.addContact(name, contact6D)
    ...
    def update(self, q, v, f, xs_0=None, us_0=None):
        if self.i == 10:
            self._mpc.step(q, v, f, xs_0=xs_0, us_0=us_0)
            self._xref = self._mpc.get_xopt()
            self._uff = self._mpc.get_uopt()
            self._Kref = self._mpc.get_Kopt()
            self._fref = self._mpc.get_fopt()
